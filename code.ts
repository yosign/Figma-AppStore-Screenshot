...
// 导入付费相关函数
import { checkUsagePermission, recordUsage, handleSubscription } from './payments';

figma.showUI(__html__, { width: 300, height: 400, title: "Super Screenshot" });

// 搜索 App Store 应用的函数
async function searchAppStore(term, country = 'us') {
  try {
...
// 从网页获取截图的函数
async function getScreenshotsFromWebpage(appId, appName, country) {
  try {
    // 构建URL
...
// 获取 Qimai 截图的函数
async function getQimaiScreenshots(appId, country = 'us') {
  try {
...
async function getAppStoreData(appId, country = 'us') {
  try {
    ...
    if (selectedScreenshots.length === 0) {
      ...
      const qimaiScreenshots = await getQimaiScreenshots(appId, country);
    ...
    if (selectedScreenshots.length === 0) {
      ...
      for (const tryCountry of countries) {
        try {
          ...
          const screenshots = await getScreenshotsFromWebpage(appId, appName, tryCountry);
          ...
          // 如果使用应用名称失败，尝试直接使用ID
          const screenshotsWithId = await getScreenshotsFromWebpage(appId, '', tryCountry);
...
// 创建布局的函数
async function createLayout(appData, config) {
  // 创建主 Frame
  ...
  // 调整图标容器大小
  iconContainer.resize(
    containerWidth,
    metaText.y + metaText.height
  );
  ...
  if (appData.screenshotUrls.length === 0) {
    ...
    screenshotsFrame.resize(
      Math.max(noScreenshotsText.width + config.spacing * 2, MIN_SIZE),
      Math.max(noScreenshotsText.height + config.spacing * 2, MIN_SIZE)
    );
  } else {
    // 计算合适的截图显示尺寸
    ...
    screenshotsFrame.resize(
      Math.max(containerWidth, MIN_SIZE),
      Math.max(containerHeight, MIN_SIZE)
    );
  }
  ...
  const frameWidth = Math.max(
    screenshotsFrame.width + config.spacing * 2,
    iconContainer.width + config.spacing * 2,
    MIN_SIZE
  );
  const frameHeight = Math.max(
    screenshotsFrame.y + screenshotsFrame.height + config.spacing,
    MIN_SIZE
  );
...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'check-permission') {
    ...
    // 处理订阅
    const success = await handleSubscription(msg.planType);
    figma.ui.postMessage({
    ...
    try {
      const results = await searchAppStore(msg.term, msg.country);
      // 发送搜索结果回UI
      figma.ui.postMessage({
        type: 'search-results',
        results: results
    ...
    try {
      ...
      const appData = await getAppStoreData(msg.appId, msg.country);
      ...
      // 刷新权限状态
      const newPermissionStatus = await checkUsagePermission();
      figma.ui.postMessage({
        type: 'permission-status',
